...
public interface LessonDaoQueries {
    ...
    String getLessonCountTillTodayByStudent = "select count(l.id) from \"Lesson\" l join \"Course\" c on l.course_id = c.id join \"Group\" g on c.id = g.course_id join \"User\" u on g.user_id = u.id join \"TimeSlot\" ts on l.time_slot_id = ts.id where u.id = ? and l.lesson_date <= cast(current_timestamp as date) and (l.is_cancel = false or l.is_cancel is null)";
